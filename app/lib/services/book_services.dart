...
class BookService {
  ...
  Future<Map<String, dynamic>> searchBooks({List<String>? cat, String? kw, bool? pmt, int? pg, bool? bf, int? py, String? pub, String? bbid, String? cls, String? asc}) async {
    // Make a GET request to the server
    ...
    var queryParams = {
      if (cat != null) 'cat': cat.join(','),
      if (kw != null) 'kw': kw,
      if (pmt != null) 'pmt': pmt.toString(),
      if (pg != null) 'pg': pg.toString(),
      if (bf != null) 'bf': bf.toString(),
      if (py != null) 'py': py.toString(),
      if (pub != null) 'pub': pub,
      if (bbid != null) 'bbid': bbid,
      if (cls != null) 'cls': cls,
      if (asc != null) 'asc': asc,
    };

    final r = await http.get(
      Uri.https(url, '/books/search', queryParams),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
    );
