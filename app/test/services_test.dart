...
Future<void> main() async {
  ...
  group('User authentication', () {
    test('registerUser returns a token if the user is registered', () async {
      final token = await userService.registerUser(
          'testuser', 'test@test.com', '1234567890', 'password', true);
      expect(token, isNotNull);
      expect(token, isA<String>());
    });

    test('registerUser throws an exception when an username is already taken',
        () async {
      try {
        await userService.registerUser(
            'testuser', 'test2@test.com', '1234567890', 'password', true);
      } catch (e) {
    ...
    test('registerUser throws an exception when an email is already taken',
        () async {
      try {
        await userService.registerUser(
            'testuser2', 'test@test.com', '1234567890', 'password', true);
      } catch (e) {
        expect(e, isA<Exception>());
    ...
    test('register a bunch of other users', () async {
      for (var i = 2; i < 7; i++) {
        final token = await userService.registerUser(
            'testuser$i', 'test$i@test.com', '1234567890', 'password', true);
        expect(token, isNotNull);
        expect(token, isA<String>());
      }
    });
    ...
    test('Update some users with notification key words', () async {
      for (var i in [3, 5]) {
        ...
        final user =
            await userService.updateUser(token, keyWords: 'Victor,Dixen');
  ...
  group('Book manipulation', () {
    test('Add some bookboxes', () async {
      ...
      final bb1 =
          await bookService.addNewBB('BB1', 0.0, 0.0, 'BB1 info', token);
      ...
      final bb2 =
          await bookService.addNewBB('BB2', 0.1, 0.1, 'BB2 info', token);
    ...
    test('Get info from ISBN, then add book by user', () async {
      ...
      expect(
          bookInfo['title'], 'Le cas Jack Spark (Saison 2) - Automne traqué');

      final book = await bookService.addBookToBB('b$qrCode', bb1Id,
          token: myToken,
          isbn: '9782075023986',
    ...
    test(
        'See if a notification has been sent to users with the correct notification key words',
        () async {
      for (var i = 3; i < 7; i++) {
        final token = await userService.loginUser('testuser$i', 'password');
        ...
        if (i == 3 || i == 5) {
          ...
          expect(
              user['user']['notifications']
                  [user['user']['notifications'].length - 1]['content'],
              'The book "Le cas Jack Spark (Saison 2) - Automne traqué" has been added to the bookbox "BB1" !');
        } else {
    ...
    test('Get book from bookbox', () async {
      final response =
          await bookService.getBookFromBB('b0', bb1Id, token: myToken);
      expect(response, isNotNull);
      expect(response, isA<Map<String, dynamic>>());
      expect(response['book']['title'],
          'Le cas Jack Spark (Saison 2) - Automne traqué');
      expect(response['books'].length, 0);
    });

    test(
        'Check if a notification was sent to the users having this book as one of their favorites',
        () async {
      final user = await userService.getUser(myToken);
      expect(user, isNotNull);
      expect(user, isA<Map<String, dynamic>>());
      expect(user['user']['notifications'].length, 1);
      expect(user['user']['notifications'][0]['content'],
          'The book "Le cas Jack Spark (Saison 2) - Automne traqué" has been removed from the bookbox "BB1" !');
    });
    ...
    test('Add a bunch of books', () async {
      ...
      for (int i = 0; i < isbns.length; i++) {
        ...
        final book = await bookService.addBookToBB('b$qrCode', bbids[i % 2],
            token: myToken,
            isbn: isbns[i],
            title: bookInfo['title'],
            authors: bookInfo['authors'] != null
                ? (bookInfo['authors'] as List<dynamic>).cast<String>()
                : <String>[],
            description: bookInfo['description'],
            coverImage: bookInfo['coverImage'],
            publisher: bookInfo['publisher'],
            ...
            categories: bookInfo['categories'] != null
                ? (bookInfo['categories'] as List<dynamic>).cast<String>()
                : <String>[]);
        expect(book, isNotNull);
        expect(book, isA<Map<String, dynamic>>());
        expect(book['bookId'], isA<String>());
    ...
    test(
        'Search specific books in specific order : '
        'in a specific book box, in the ascending alphabetical order of the 1st author\'s name',
        () async {
      final response = await bookService.searchBooks(
          cls: 'by author', asc: true, bbid: bb1Id);
      expect(response, isNotNull);
      expect(response, isA<Map<String, dynamic>>());
      for (var i = 0; i < response['books'].length - 1; i++) {
        expect(
            response['books'][i]['authors'][0]
                .compareTo(response['books'][i + 1]['authors'][0]),
            lessThanOrEqualTo(0));
      }
    ...
    test(
        'Remove a book from the bookbox then expect the result to have 1 book less',
        () async {
      final response =
          await bookService.getBookFromBB('b1', bb1Id, token: myToken);
      final response2 = await bookService.searchBooks(
          cls: 'by author', asc: true, bbid: bb1Id);
      expect(response, isNotNull);
    ...
    test('Same as earlier but in descending order', () async {
      final response = await bookService.searchBooks(
          cls: 'by author', asc: false, bbid: bb1Id);
      expect(response, isNotNull);
      expect(response, isA<Map<String, dynamic>>());
      for (var i = 0; i < response['books'].length - 1; i++) {
        expect(
    ...
    test(
        'Search specific books in specific order : '
        'published after 2016, in the ascending alphabetical order of the title',
        () async {
      final response = await bookService.searchBooks(
          cls: 'by title', asc: true, bf: false, py: 2016);
      expect(response, isNotNull);
      expect(response, isA<Map<String, dynamic>>());
      for (var i = 0; i < response['books'].length - 1; i++) {
        expect(
            response['books'][i]['title']
                .compareTo(response['books'][i + 1]['title']),
            lessThanOrEqualTo(0));
      }
    });
    ...
    test(
        'Search specific books in specific order : '
        'whose title or author contains "Harry", in the descending order of the parution year',
        () async {
      final response = await bookService.searchBooks(
          cls: 'by year', asc: false, kw: 'Harry');
      expect(response, isNotNull);
      expect(response, isA<Map<String, dynamic>>());
      for (var i = 0; i < response['books'].length - 1; i++) {
        expect(
            response['books'][i]['title'].contains('Harry') ||
                response['books'][i]['authors'].contains('Harry'),
            true);
        expect(
            response['books'][i]['parutionYear']
                .compareTo(response['books'][i + 1]['parutionYear']),
            greaterThanOrEqualTo(0));
      }
    });
    ...
    test(
        'Search specific books in specific order : '
        'whose publisher is VIZ Media LLC, in the ascending order of the most recent activity',
        () async {
      final response = await bookService.searchBooks(
          pub: 'VIZ Media LLC', cls: 'by most recent activity', asc: true);
      expect(response, isNotNull);
      expect(response, isA<Map<String, dynamic>>());
      for (var i = 0; i < response['books'].length - 1; i++) {
        ...
        final date2 =
            DateTime.parse(response['books'][i + 1]['dateLastAction']);
  ...
  group('Thread manipulation', () {
    test('Create a thread', () async {
      final response =
          await threadService.createThread(myToken, bid, 'Test thread');
      expect(response, isNotNull);
      expect(response, isA<Map<String, dynamic>>());
    ...
    test('Add a message to the thread', () async {
      final response =
          await threadService.addMessage(myToken, tid, 'Test message');
      expect(response, isNotNull);
      expect(response, isA<Map<String, dynamic>>());
    ...
    test('Add a message that responds to another message', () async {
      final response = await threadService
          .addMessage(myToken, tid, 'Test response', respondsTo: mid);
      expect(response, isNotNull);
      expect(response, isA<Map<String, dynamic>>());
    ...
    test('Add a reaction to the message', () async {
      final reaction = await threadService.toggleMessageReaction(
          myToken, tid, mid, 'link/to/emoji');
