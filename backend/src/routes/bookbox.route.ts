...
import { BookboxService } from "../services";
import {
    addBookToBookboxSchema,
    getBookFromBookBoxSchema,
    removeBookFromBookBoxSchema,
    getBookboxSchema,
    followBookBoxSchema,
    unfollowBookBoxSchema,
} from "../schemas";
import { BookAddData } from "../types";
...
export default async function bookBoxRoutes(server: MyFastifyInstance) {
    ...
    server.delete('/bookboxes/:bookboxId/books/:bookId', { preValidation: [server.bookManipAuth, server.optionalAuthenticate], schema: removeBookFromBookBoxSchema }, getBookFromBookBox);
